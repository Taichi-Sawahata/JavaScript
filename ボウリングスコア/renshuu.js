...
//変数に代入されたボタンがクリックされたら、関数のパラメータに代入することで
//クリックされたときに押されたボタンを呼び出せ、handleEventメソッドによって関数が実行される
...
//イベントリスナーの第二引数をオブジェクトにし、1つ目のプロパティの値をテンプレート文字列
//にすることで、押したボタンがhtmlに表示されるようにする
  


function roll(){
  //空の配列に押したボタンのvalueを入れていく
  //押したボタンをthisで指定する事ができ、それを整数に直し、配列に入れていく
  ...
  //score数に配列i番目の値を代入
    ...
    //最終フレーム　breakすることにより、下のif文が読み込まれない
 ...
 function sum(){
   ...
   let list = $("td[class=goukei]").map(function(index, val){
    //テキストとして表示されたvalを整数に直す
     let kazu = parseInt($(val).text());
     //kazuが0より大きければ、値を返す　returnの値を設定しておくことで、呼び出し時に値を返してくれる
      if(kazu >= 0) {
        return kazu;
      } else {
        return null;
    ...
    //繰り返し処理をしたいコールバック関数index,val(オブジェクトであればkey);
